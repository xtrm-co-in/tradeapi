...
from .serializers import StockoutwardSerializer,SalesSerializer,MenuitemSerializer,UserSerializer
from .models import Stockoutward,Sales,Menuitems,User
...
class SalesViewSet(mixins.ListModelMixin,viewsets.GenericViewSet):
    ...
    def list(self,request):
        # start_dt=self.request.query_params.get('vdate_after')
        # end_dt=self.request.query_params.get('vdate_before')
        # serializer=SalesSerializer(Sales.objects.filter(author=self.request.user,vdate__range=(start_dt,end_dt)),many=True)
